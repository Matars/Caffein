...
def seed_fire_detections(session, json_files, batch_size=5000, limit_per_file=None):
    """Seed fire detection data from JSON files into PostgreSQL"""
    ...
    try:
                ...
                with open(json_file, 'rb') as f:
                    ...
                    for record in records_stream:
                        ...
                        if limit_per_file and records_processed >= limit_per_file:
                            print(f"  ⚠️  Reached limit of {limit_per_file:,} records")
                            break
                        ...
                        try:
                            acq_date = datetime.strptime(record['acq_date'], '%Y-%m-%d').date()
                        except:
                            total_skipped += 1
                            continue
                        ...
                        detection = FireDetection(
                            ...
                            confidence=str(record.get('confidence')) if record.get('confidence') else None,
                            frp=float(record['frp']) if record.get('frp') else None,
                            brightness=float(record['brightness']) if record.get('brightness') else None,
                            bright_t31=float(record.get('bright_t31')) if record.get('bright_t31') else None,
                            instrument=record.get('instrument'),
                            satellite=record.get('satellite'),
                            version=record.get('version'),
                            daynight=record.get('daynight'),
                            type=str(record.get('type')) if record.get('type') else None,
                            scan=float(record.get('scan')) if record.get('scan') else None,
                            track=float(record.get('track')) if record.get('track') else None
                        )
                        ...
                        if len(batch_records) >= batch_size:
                            ...
                            print(f"    Progress: {total_inserted:,} records inserted")
                            batch_records = []
                    ...
                    print(f"  ✓ Completed: {records_processed:,} records processed from {json_file.name}")
        ...
        print(f"\n✓ Inserted {total_inserted:,} fire detection records.")
        if total_skipped > 0:
            print(f"  ⚠️  Skipped {total_skipped:,} records (invalid data)")
        return True
...
def main():
    ...
    fire_success = seed_fire_detections(session, json_files, limit_per_file=limit_per_file)
