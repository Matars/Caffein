...
class FireDetectionSweden(Base):
    """NASA FIRMS satellite fire detection model for Sweden"""
    __tablename__ = 'fire_detections_sweden'
    ...
    scan = Column(Numeric(10, 2))
    track = Column(Numeric(10, 2))
    grid_lat_idx = Column(Integer)  # Grid cell latitude index
    grid_lon_idx = Column(Integer)  # Grid cell longitude index
    created_at = Column(TIMESTAMP, server_default=func.current_timestamp())
...
class NO2MeasurementSweden(Base):
    ...
    # Tropospheric NO2 column density (molecules/cm²)
    no2_column = Column(Numeric(15, 6))
